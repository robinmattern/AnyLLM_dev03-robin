...
async function chatSync({
  ...
  const textResponse = await LLMConnector.getChatCompletion(messages, {                                     // .(41005.05.16)
    temperature:
...
async function streamChat({
  ...
  const stream = await LLMConnector.streamGetChatCompletion(messages, {                                     // .(41005.05.20)
    temperature:
      temperature ?? workspace?.openAiTemp ?? LLMConnector.defaultTemp,
  });
  const completeText = await LLMConnector.handleStream(                                                     // .(41005.05.21)
    responseInterceptor,
    stream,
    {
      uuid,
