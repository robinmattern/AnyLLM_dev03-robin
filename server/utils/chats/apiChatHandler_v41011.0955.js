...
async function chatSync({
  ...
  const textResponse = await LLMConnector.getChatCompletion(messages, {                                     // .(41005.05.3)
  ...
  if (LLMConnector.streamingEnabled() !== true) {
    console.log(
      `\x1b[31m[STREAMING DISABLED]\x1b[0m Streaming is not available for ${LLMConnector.constructor.name}. Will use regular chat method.`
        ...
        completeText = await LLMConnector.getChatCompletion(messages, {                                     // .(41005.05.7)
      temperature: workspace?.openAiTemp ?? LLMConnector.defaultTemp,
    });
    ...
    const stream = await LLMConnector.streamGetChatCompletion(messages, {                                   // .(41005.05.11)
      temperature: workspace?.openAiTemp ?? LLMConnector.defaultTemp,
    });
        completeText = await LLMConnector.handleStream(response, stream, {                                  // .(41005.05.12)
      uuid,
      sources,
    });
  }
